...
export const StockTable = ({ 
  ...
  const getStatusClasses = (status: StockItem['status']) => (
    status === "In Stock"
  ...
  return (
    <div className="space-y-4 w-full">
      <div className="space-y-2 sm:hidden">
        {items.length > 0 ? (
          items.map((item) => (
            <div key={`mobile-${item.id}`} className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-slate-800">{item.name}</p>
                  <p className="mt-0.5 text-xs text-slate-500">{item.category}</p>
                </div>
                <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${getStatusClasses(item.status)}`}>
                  {item.status}
                </span>
              </div>
              ...
              <div className="mt-2 grid grid-cols-2 gap-2 text-xs text-slate-600">
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-wide text-slate-400">Min Stock</p>
                  <p className="font-medium text-slate-700">{item.minimumStock}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wide text-slate-400">Added / Issued</p>
                  <p className="font-medium text-slate-700">+{item.numberAdded} / -{item.numberIssued}</p>
                </div>
                <div>
                  <p className="text-[10px] uppercase tracking-wide text-slate-400">Date</p>
                  <p className="font-medium text-slate-700">{new Date(item.date).toLocaleDateString()}</p>
                </div>
              </div>
              ...
              <div className="mt-3 flex items-center justify-end gap-1.5">
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 w-8 p-0 rounded-md border border-green-200 bg-green-50 hover:bg-green-100 shadow-sm"
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 w-8 p-0 rounded-md border border-orange-200 bg-orange-50 hover:bg-orange-100 shadow-sm"
                ...
                <Button
                  variant="outline"
                  size="sm"
                  className="h-8 w-8 p-0 rounded-md border border-purple-200 bg-purple-50 hover:bg-purple-100 shadow-sm"
                  onClick={() => onEdit(item)}
                ...
                <Button
                  ...
                  className="h-8 w-8 p-0 rounded-md border border-red-200 bg-red-50 hover:bg-red-100 shadow-sm"
      ...
      <div className="hidden rounded-md border w-full overflow-x-auto sm:block">
                ...
                <TableCell className="font-medium py-3">
                  <div className="flex flex-col sm:flex-row sm:items-center">
                    ...
                    <span className="sm:hidden mt-1">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${getStatusClasses(item.status)}`}>
                        {item.status}
                      </span>
                    </span>
                  </div>
